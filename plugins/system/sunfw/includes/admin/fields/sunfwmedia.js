			...
			init: function() {
				...
				$('.sunfw-media-selector a[href="#sunFwModalMedia"]').click(
					function(event) {
						...
						setTimeout(function() {
							var viewPortHeight = window.innerHeight || (document.documentElement || document.body).clientHeight;
							var modalRect = self.modal.getBoundingClientRect();
							var modalHeight = viewPortHeight - (modalRect.top * 2);
							var modalHeaderHeight = self.modal.querySelector('.modal-header').getBoundingClientRect().height;
							var modalFooterHeight = self.modal.querySelector('.modal-footer').getBoundingClientRect().height;
							var modalBody = self.modal.querySelector('.modal-body');
							var modalBodyCss = window.getComputedStyle(modalBody);
							var modalBodyHeight = modalHeight - modalHeaderHeight - modalFooterHeight;
							var iframeCss = window.getComputedStyle(self.iframe);
							var iframeHeight = modalBodyHeight
