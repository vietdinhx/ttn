...
class SunFwAjaxUserAccount extends SunFwAjax
{
	...
	public function copyTokenFromAction()
	{
		...
		// Get the parameters of the current template.
		$current = SunFwHelper::getExtensionParams('template', $this->template['name']);

		// Clear discount offer if necessary.
		$update = array(
		...
		if (
			!empty($current['lastUser'])
			&&
			!empty($current['clickedPurchase'])
			&&
			$current['lastUser'] !== $params['username']
		) {
			$update['clickedPurchase'] = null;
			$update['currentDiscount'] = null;
		}

		// Store token key.
		SunFwHelper::updateExtensionParams($update, 'template', $this->template['name']);
	...
	public function getTokenKeyAction()
	{
		...
		preg_match(
			'@^(?:http://www\.|http://|www\.|http:|https://www\.|https://|www\.|https:)?([^/]+)@i',
			$domain,
			$domainFilter
		);
		...
		try
		{
			...
			{
				throw new Exception(JText::_('SUNFW_LIGHTCART_ERROR_' . strtoupper(preg_replace(
					'/[^a-zA-Z0-9_]+/', '_', $result['message']
				))));
			}
			...
			if (
				!empty($params['lastUser'])
				&&
				!empty($params['clickedPurchase'])
				&&
				$params['lastUser'] !== $username
			) {
				$update['clickedPurchase'] = null;
				$update['currentDiscount'] = null;
			}
