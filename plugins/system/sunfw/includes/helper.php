...
class SunFwHelper
{
	...
	public static function getLayoutItemList($styleID)
	{

	    $style = self::getOnlySunFwStyle($styleID);

	    $result = array();
	    if ($style)
	    {
	        $layout = $style->layout_builder_data;
                ...
                if (count($sections))
                {
                    ...
                    foreach ($sections as $section)
                    {
                        ...
                        if (count($tmpRows))
                        {
                            ...
                            {
                                if (isset($rows[$tmpRow]))
                                {
                                    $rowElement = $rows[$tmpRow];
                                    $tmpColumns = $rowElement['columns'];
                                    if (count($tmpColumns))
                                    {
                                        foreach ($tmpColumns as $tmpColumn)
                                        {
                                            if (isset($columns[$tmpColumn]))
                                            {
                                                $rowColumn = $columns[$tmpColumn];
                                                $tmpItems = $rowColumn['items'];
                                                if (count($tmpItems))
                                                {
                                                    foreach ($tmpItems as $tmpItem)
                                                    {
                                                        if (isset($items[$tmpItem]))
                                                        {
                                                            $itemElement = $items[$tmpItem];

                                                            $itemStd = new stdClass();
                                                            $itemStd->id = $itemElement['id'];
                                                            $itemStd->type = $itemElement['type'];
                                                            $itemStd->section_id = $sectionID;
                                                            $result [] = $itemStd;
                                                        }
                                                    }
                                                }
	...
	public static function getExtensionInfo($element, $type, $folder="")
	{
